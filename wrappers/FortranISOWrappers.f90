...
CONTAINS
  ...
  SUBROUTINE directeos__notidgas_ctor_sub_wrap(instance, ident, nc, Tc, Vc) &
    &BIND(C)
    ...
    END SELECT
  END SUBROUTINE directeos__notidgas_ctor_sub_wrap

  SUBROUTINE directeos__notidgas_fres_wrap(c_this, T, V, n, Fres, Ft, Fv, Fn&
    &) BIND(C)
    TYPE(C_PTR), VALUE :: c_this
    REAL(C_FLOAT) :: T
    REAL(C_FLOAT) :: V
    ...
    END SELECT
  END SUBROUTINE directeos__notidgas_fres_wrap

END MODULE directeos_wrap_
